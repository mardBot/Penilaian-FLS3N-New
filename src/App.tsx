...
export default function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch Schools
        const { data: schoolsData, error: schoolsError } = await supabase.from('master_schools').select('name').order('name');
        if (schoolsData && schoolsData.length > 0) {
          setSchoolList(schoolsData.map(s => s.name));
        } else {
          setSchoolList([...SCHOOL_LIST].sort());
          if (!schoolsError) {
            await supabase.from('master_schools').insert(SCHOOL_LIST.map(name => ({ name })));
          }
        }

        // Fetch Categories
        const { data: categoriesData, error: categoriesError } = await supabase.from('master_categories').select('name').order('name');
        if (categoriesData && categoriesData.length > 0) {
          setCabangLomba(categoriesData.map(c => c.name));
        } else {
          setCabangLomba([...CABANG_LOMBA].sort());
          if (!categoriesError) {
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'peserta' }, (payload) => {
        if (payload.eventType === 'INSERT') {
          ...
          setPesertaList(prev => prev.map(p => p.id === updatedP.id ? {
            ...
            status: updatedP.status || "Pending",
    ...
    const settingsSubscription = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'settings' }, (payload) => {
        ...
        setSettings({
          ...
          headOfDepartmentName: s.head_of_department_name || "TRI KRISNI ASTUTI, S.Sos, MM.",
          headOfDepartmentNip: s.head_of_department_nip || "197004241997032007",
          categoryJudges: s.category_judges || {},
          logoKabupaten: s.logo_kabupaten || "",
          logoFLS3N: s.logo_fls3n || ""
        });
      })
      .subscribe();
  ...
  const confirmDelete = async () => {
    if (deletingId) {
      const deletedPeserta = pesertaList.find(p => p.id === deletingId);
  ...
  const handleImportCSV = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      ...
      const importedPeserta = data.map((item) => ({
      ...
      if (importedPeserta.length > 0) {
        const { data: insertedData, error } = await supabase.from('peserta').insert(importedPeserta.map(p => ({
          name: p.name,
          school: p.school,
