...
async function testInsertPeserta() {
  const { data, error } = await supabase.from('peserta').insert([{ id: 'test', name: 'Test', school: 'Test', category: 'Test' }]);
  console.log('Peserta insert result:', { data, error });
}
