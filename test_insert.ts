...
async function testInsert() {
  const { data, error } = await supabase.from('master_schools').insert([{ name: 'Test School 123' }]).select();
  console.log('Insert result:', { data, error });
  
  if (!error) {
    await supabase.from('master_schools').delete().eq('name', 'Test School 123');
  }
}
